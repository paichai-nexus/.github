...
> **2026 · 53 Members · 21 Fields · 13 Project / TF · Matrix Organization**  
...
- 비서는 랩장 직속으로 정보·기록·일정·후속조치를 지원합니다.
...
## 11. 🤖 Campus Autonomous Delivery Robot TF

**컴퓨터공학 × 드론로봇공학 × 전기전자공학 × 산업디자인 × Smart Mobility**

### ROS 2 기반 캠퍼스 자율주행·배송 로봇 시스템

배재대학교 캠퍼스를 실제 실증환경으로 활용하여  
**자율주행 로봇의 인지·위치추정·경로계획·제어·안전정지·서비스 연동**을 통합적으로 연구·개발하는 프로젝트입니다.

단순히 로봇을 움직이는 데 그치지 않고, 실제 캠퍼스 환경에서 운용 가능한 시스템을 목표로  
ROS 2 기반 시스템 아키텍처와 안전 요구사항부터 단계적으로 구축합니다.

**Core System**

`ROS 2` · `Nav2` · `Localization` · `Perception` · `Path Planning` · `Obstacle Avoidance` · `Fail-Safe`
...
Safety & Fail-Safe
        ↓
Campus Validation Scenario
        ↓
Paejae Pick / Smart Mobility Integration
```
...
| 분야 | 역할 |
| --- | --- |
| 컴퓨터공학 / 소프트웨어 | ROS 2, Navigation, 시스템 통합, Backend, 데이터 처리 |
| 드론로봇공학 | 자율주행, 센서융합, 제어, 로봇 플랫폼 |
| 전기전자공학 | 센서, MCU, 전원, 통신, 하드웨어 |
| 산업디자인 / 디자인 | 로봇 외형, HMI, 서비스 UX |

**Initial Outputs**

`System Architecture` · `Campus Test Scenario` · `Safety Requirements` · `ROS 2 Navigation Prototype`

<img src="https://img.shields.io/badge/STATUS-ACTIVE%20%2F%20RESEARCH-22C55E?style=flat-square">

---

## 12. 🛰️ Space & Satellite Systems TF

**전기전자 × 드론로봇 × 스마트배터리 × Software × AI × Remote Sensing × Design × Policy × Business**

### 우주·위성 시스템 Digital Twin & Mission Control 프로젝트

배재대학교에는 별도의 우주항공학과가 없지만, 여러 전공의 기술과 도메인을 연결하여  
**위성 시스템을 직접 이해하고 구현해보는 융합형 Space Systems 프로젝트**입니다.

실제 위성 발사를 첫 목표로 두기보다,  
Satellite BUS · Ground Segment · Mission Planning · Digital Twin · Telemetry · 위성영상 활용을 단계적으로 구현하며  
시스템공학 관점에서 우주·위성 기술을 학습하고 프로젝트 결과물로 연결합니다.

### 1순위 · Core Technical Team

> **Bus + SW + Control을 구성하는 핵심 기술팀**

| 전공 | 역할 |
| --- | --- |
| 전기전자공학과 | 위성 전력계, 센서, RF, 통신, 회로, MCU |
| 드론로봇공학과 | 자세제어, 센서융합, 제어, Actuator, 무인체계, ADCS |
| 스마트배터리학과 | Battery, BMS, EPS, 전력관리 |
| 소프트웨어학부 / 컴퓨터공학 | Digital Twin, Mission Control, Backend, Telemetry, AI, Simulator |

### 2순위 · Mission / Application Team

> **‘위성으로 무엇을 할지’를 정의하고 실제 문제 해결로 연결하는 팀**

| 전공 | 역할 |
| --- | --- |
| 원예산림학과 | 식생·산림·산불·작황 위성영상 분석 |
| 생명공학과 | 우주생명과학, Bio Payload, 환경센서 응용 |
| 조경학과 | 도시녹지, 토지피복, 공간변화 관측 |
| 철도건설 / 건축 관련 학과 | GIS, 구조물·도시·인프라 원격탐사 활용 |

### 3순위 · Project Completion & Expansion

> **프로젝트의 UX·콘텐츠·정책·사업화 완성도를 높이는 팀**

| 전공 | 역할 |
| --- | --- |
| 디자인학부 | Mission Control UI/UX, 시스템 시각화 |
| 미디어콘텐츠학과 | 영상 콘텐츠, 3D 시각화, 홍보 |
| 행정학과 | 우주정책, 재난·공공데이터 활용, 공공 R&D 기획 |
| 경영학과 / IT경영정보학과 | 사업모델, 시장조사, PM, 기술사업화 |

**Development Roadmap**

```text
Satellite System Study
        ↓
Orbit / Attitude / Power Modeling
        ↓
Satellite Digital Twin
        ↓
Telemetry / Command Backend
        ↓
Mission Control Dashboard
        ↓
Mission Planning
        ↓
Satellite Image / Remote Sensing Mission
        ↓
...
- 가상 위성의 궤도·자세·전력 상태 모델링
- Telemetry / Command 데이터 흐름 구현
- React 기반 Mission Control Dashboard
- 지상국 / Ground Segment 개념 설계
- 위성영상·원격탐사 기반 Mission PoC
- 향후 센서·MCU 등을 활용한 Hardware-in-the-Loop 확장 검토

**Initial Outputs**

`Satellite Digital Twin` · `Mission Control Dashboard` · `Telemetry / Command Simulator` · `Mission Planning PoC` · `Satellite Image AI Mission`

<img src="https://img.shields.io/badge/STATUS-FORMING%20%2F%20RECRUITING-0EA5E9?style=flat-square">

---

## 13. 🤖 Nasomi Walking Robot TF

**컴퓨터공학 × 드론로봇공학 × 전기전자공학 × 산업디자인 × Robotics × Physical AI**

### 배재대학교 마스코트 '나섬이' 보행형 로봇 프로젝트

배재대학교 마스코트 **나섬이**를 실제로 움직이는 로봇 플랫폼으로 구현하여,  
교내 행사·입학홍보·전시·체험에서 활용할 수 있는 **학생 주도형 Campus Mascot Robot**을 개발하는 프로젝트입니다.

KAIST의 마스코트 로봇 사례에서 아이디어를 얻되, 배재대학교의 캐릭터·공간·학생 프로젝트 문화를 반영한 독자적인 시스템으로 발전시키는 것을 목표로 합니다.

**Core System**

`Embedded` · `Motor Control` · `Sensors` · `MCU` · `ROS 2` · `Walking / Mobility` · `HMI` · `Mechanical Design`

**Major Roles**

| 분야 | 역할 |
| --- | --- |
| 컴퓨터공학 / 소프트웨어 | ROS 2, 제어 SW, 상태관리, 센서 데이터, HMI·서비스 연동 |
| 드론로봇공학 | 로봇 구동, 제어, 센서융합, 보행·이동 메커니즘 |
| 전기전자공학 | MCU, 모터드라이버, 전원, 센서, 배선, PCB, 안전회로 |
| 산업디자인 / 디자인 | 나섬이 외형, 기구·외장, 사용자 경험, 전시·홍보 디자인 |

**1차 PoC**

```text
Character / Mechanical Concept
        ↓
Actuator & Electronics Design
        ↓
MCU / Motor Control
        ↓
Basic Standing / Walking Test
        ↓
Sensor & Safety Integration
        ↓
ROS 2 / Interaction Extension
        ↓
Campus Exhibition / Demo
```

**Initial Outputs**

`Mechanical Concept` · `Electronics / BOM` · `Motor Control PoC` · `Walking Prototype` · `Safety Checklist` · `Campus Demo`

<img src="https://img.shields.io/badge/STATUS-PLANNING%20%2F%20PROTOTYPE-8B5CF6?style=flat-square">

---

### 🚀 Project Portfolio Update · 프로젝트 현황

| 번호 | 프로젝트 | 상태 |
| ---: | --- | --- |
| **08** | 🌐 NEXUS Web Platform | `PLANNING / INTERNAL INFRA` |
| **09** | ✈️ Smart Travel & Airport Experience | `VOTING` |
| **10** | 🎭 Interactive Culture Tech Studio | `VOTING` |
| **11** | 🤖 Campus Autonomous Delivery Robot | `ACTIVE / RESEARCH` |
| **12** | 🛰️ Space & Satellite Systems | `FORMING / RECRUITING` |
| **13** | 🤖 Nasomi Walking Robot | `PLANNING / PROTOTYPE` |

> 08번은 NEXUS 내부 인프라 프로젝트로 추진합니다.  
> 09·10번은 신규 학과 참여 수요를 확인하기 위한 투표 후보 TF입니다.  
> 11번은 캠퍼스 자율주행·배송 로봇 연구 프로젝트로 진행하며,  
> 12번은 우주·위성 시스템을 여러 전공이 함께 구현하는 신규 융합 TF로 팀원을 모집합니다.
> 13번은 배재대학교 마스코트 나섬이를 실제 보행형 로봇으로 구현하는 Campus Mascot Robot 프로젝트입니다.
...
```text
        Executive Office ↔ Executive Council ↔ Project / TF Leaders
                                │
                      FUNCTIONAL DEPARTMENTS
                                │
        ┌──────────────┬──────────────┬──────────────┬──────────────┐
...
> 기능부서는 운영진의 하위 조직이 아니라 **상시 역량 풀**입니다. 프로젝트/TF가 만들어지면 필요한 인원이 각 부서에서 TF로 교차 참여합니다.
...
<sub><b>구민우</b></sub><br>
<sub>1학년 · IT경영정보</sub><br><sub><b>부랩장 · PMO Lead</b></sub>
</td>
<td align="center" width="25%" valign="top">
...
<sub><b>최민성</b></sub><br>
<sub>1학년 · IT경영정보</sub><br><sub><b>Secretary · Planning Support</b></sub>
</td>
<td align="center" width="25%" valign="top">
...
<p align="center"><b>PAICHAI NEXUS</b><br>EXECUTIVE COUNCIL · PROJECT / TF · FUNCTION · MAJOR NETWORK</p>
